...
public class Sound
{
	...
	//New addition for loaded sounds >> Juwal
	public Sound(AudioClip newSound) {
		if (newSound == null)
			throw new Exception("Audio clip is null");
		name = newSound.name;
		clip = newSound;
	}
	//
    ...
    public void PlayOrPause(bool play, bool pauseOthers) {
        ...
        if (play && !source.isPlaying) {
			//source.volume=UserDataManager.Instance.currentUser.soundVolume;
            //if(!UserDataManager.Instance.currentUser.isMuted)
				source.Play();
        } else {
            source.Pause();
        }

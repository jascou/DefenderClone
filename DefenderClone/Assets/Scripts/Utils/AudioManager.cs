...
public class AudioManager : MonoBehaviour
{
	...
	//Added for already loaded sounds
	/// Creates a new sound, registers it, gives it the properties specified, and starts playing it
	public Sound PlayNewSound(AudioClip soundClip, bool loop=false, bool interrupts=false, Action<Sound> callback=null) {
		//if (!UserDataManager.Instance.currentUser.isSoundEnabled)
			//return null;

		Sound sound = NewSound(soundClip, loop, interrupts, callback);
		sound.playing = true;
		return sound;
	}
	
	/// Creates a new sound, registers it, and gives it the properties specified
	public Sound NewSound(AudioClip soundClip, bool loop=false, bool interrupts=false, Action<Sound> callback=null) {
		Sound sound = new Sound(soundClip);
		RegisterSound(sound);
		sound.loop = loop;
